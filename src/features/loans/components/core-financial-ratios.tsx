      ...
      <CardContent className="pt-5 pb-5">
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="rounded-lg p-4 bg-gradient-to-br from-blue-500/10 to-blue-600/5 border border-blue-500/20">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-semibold text-blue-600">
                P/Loan (Price-to-Loan)
              </span>
            </div>
            <div className="text-2xl font-bold text-foreground mb-1">
              {ratios.pToLoan.toFixed(2)}x
            </div>
            <div className="text-xs text-muted-foreground">
              มูลค่าทรัพย์สินกว่าเงินกู้ {ratios.pToLoan.toFixed(2)} เท่า แสดงถึงศักยภาพดี
            </div>
          </div>
          ...
          <div className="rounded-lg p-4 bg-gradient-to-br from-amber-500/10 to-amber-700/5 border border-amber-500/20">
            ...
            <div className="text-2xl font-bold text-foreground mb-1">
              {ratios.ytdGap.toFixed(1)}% {ratios.ytdGapDirection}
            </div>
            <div className="text-xs text-muted-foreground">
              ส่วนต่างระหว่างจริงและแผน (
              {ratios.ytdGapDirection === 'lag' ? 'ต่ำกว่า' : 'สูงกว่า'} {ratios.ytdGap.toFixed(1)}
              %)
            </div>
          </div>
          ...
          <div className="rounded-lg p-4 bg-gradient-to-br from-indigo-500/10 to-indigo-600/5 border border-indigo-500/20">
            ...
            <div className="text-2xl font-bold text-foreground mb-1">
              {ratios.irr > 0 ? `${ratios.irr.toFixed(1)}%` : 'ยังไม่มีข้อมูล'}
            </div>
            <div className="text-xs text-muted-foreground">
        ...
        <div className="mt-5 p-4 bg-accent/30 rounded-lg border border-border">
          ...
          <div className="space-y-2 text-xs text-muted-foreground leading-relaxed">
            ...
            {ratios.ltv !== null ? (
              ...
              <p>
                • <span className="font-medium text-foreground">ยังไม่มีข้อมูล LTV</span>{' '}
            ...
            <p>
              ...
              แปลว่าดอกเบี้ยจริง ณ ปัจจุบันนี้ ยังคงอยู่ในระดับดอกเบี้ยเสนอ แม้จะต่ำกว่าผลตอบแทนเช่น
              36% แต่ก็ยังดี (Gap 2.8%)
            </p>
            <p>
              •{' '}
              <span className="font-medium text-foreground">
            ...
            <p>
              ...
              หมายความว่าไม่มีผลิตภัณฑ์สะสมระยะสั้น เนื่องจากเลือกการผ่อนชำระระยะยาว
            </p>
          </div>
        </div>
      </CardContent>
