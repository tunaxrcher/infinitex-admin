...
function AiAppraisalSection({
  ...
  const confidence = manualValue > 0 && aiValue > 0
    ? Math.max(40, Math.min(99, Math.round(100 - (Math.abs(aiValue - manualValue) / manualValue) * 100)))
    : 0;
  const confidenceLabel = confidence >= 80 ? 'สูง' : confidence >= 60 ? 'ปานกลาง' : 'ต่ำ';
  const confidenceColor = confidence >= 80 ? '#16a34a' : confidence >= 60 ? '#d97706' : '#dc2626';
  const diff     = aiValue - manualValue;
  const diffPct  = manualValue > 0 ? ((diff / manualValue) * 100).toFixed(1) : '0.0';
  const diffSign = diff >= 0 ? '+' : '';
  const diffColor = diff >= 0 ? '#16a34a' : '#dc2626';
  const ltv = loanPrincipal > 0 && aiValue > 0
    ? ((loanPrincipal / aiValue) * 100).toFixed(1) : null;

  const factors = [
    { label: `ทำเล: ${placeText.slice(0, 18)}`, impact: '+', color: '#16a34a' },
    { label: `ประเภท: ${(propertyType || 'ที่ดิน').slice(0, 14)}`, impact: '≈', color: '#6b7280' },
    { label: `LTV: ${ltv ? ltv + '%' : '-'}`, impact: ltv && Number(ltv) < 70 ? '+' : '–', color: ltv && Number(ltv) < 70 ? '#16a34a' : '#d97706' },
  ];
    ...
    >
      ...
      >
        <PdfView style={{ flexDirection: 'row', alignItems: 'center' }}>
          <PdfText style={{ fontSize: 8.5, fontWeight: 700, color: T.indigoDark }}>
      ...
      <PdfView style={{ flexDirection: 'row', backgroundColor: T.bg }}>
        ...
        <PdfView style={{ flex: 1, padding: 6, borderRightWidth: 1, borderRightColor: '#e0e7ff', borderRightStyle: 'solid' }}>
          <PdfText style={{ fontSize: 7, color: T.indigoText, fontWeight: 700, marginBottom: 2 }}>AI ราคาประเมิน</PdfText>
          <PdfText style={{ fontSize: 15, fontWeight: 700, color: '#1e1b4b', lineHeight: 1.1 }}>
            ฿{formatCurrency(aiValue)}
          </PdfText>
          <PdfText style={{ fontSize: 6.5, color: T.muted, marginTop: 1 }}>{toThaiBahtText(aiValue)}</PdfText>
          {manualValue > 0 && (
            <PdfView style={{ marginTop: 4, flexDirection: 'row', alignItems: 'center', gap: 4 }}>
              <PdfText style={{ fontSize: 7, color: T.muted }}>vs Manual:</PdfText>
              <PdfText style={{ fontSize: 7.5, fontWeight: 700, color: diffColor }}>
                {diffSign}{diffPct}%
              </PdfText>
            </PdfView>
          )}
        ...
        <PdfView style={{ flex: 1, padding: 6, borderRightWidth: 1, borderRightColor: '#e0e7ff', borderRightStyle: 'solid' }}>
          <PdfText style={{ fontSize: 7, color: T.indigoText, fontWeight: 700, marginBottom: 2 }}>ความเชื่อมั่น (Confidence)</PdfText>
          <PdfView style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 4 }}>
            <PdfText style={{ fontSize: 16, fontWeight: 700, color: T.text, lineHeight: 1 }}>{confidence}</PdfText>
            <PdfText style={{ fontSize: 10, color: T.muted, marginLeft: 1, marginBottom: 1 }}>%</PdfText>
            <PdfView style={{ marginLeft: 5, backgroundColor: confidenceColor + '20', borderRadius: 6, paddingHorizontal: 4, paddingVertical: 1 }}>
              <PdfText style={{ fontSize: 6.5, color: confidenceColor, fontWeight: 700 }}>{confidenceLabel}</PdfText>
            </PdfView>
          </PdfView>
        ...
        <PdfView style={{ flex: 1, padding: 6 }}>
          <PdfText style={{ fontSize: 7, color: T.indigoText, fontWeight: 700, marginBottom: 3 }}>ปัจจัยสำคัญที่ใช้ประเมิน</PdfText>
          {factors.map((f, i) => (
            <PdfView key={i} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 3 }}>
              <PdfView style={{ width: 12, height: 12, borderRadius: 6, backgroundColor: f.color + '20', justifyContent: 'center', alignItems: 'center', marginRight: 4 }}>
                <PdfText style={{ fontSize: 7, color: f.color, fontWeight: 700 }}>{f.impact}</PdfText>
              </PdfView>
              <PdfText style={{ flex: 1, fontSize: 7.5, color: T.text }}>{f.label}</PdfText>
            </PdfView>
          ))}
          <PdfView style={{ marginTop: 3, backgroundColor: '#fef9c3', borderRadius: 2, padding: 4 }}>
            <PdfText style={{ fontSize: 6.5, color: '#713f12', lineHeight: 1.3 }}>
              ⚠️ ผลประเมิน AI ใช้ประกอบการตัดสินใจเท่านั้น ต้องผ่านการรับรองผู้ประเมินที่ได้รับอนุญาต
            </PdfText>
          </PdfView>
        </PdfView>
      </PdfView>
...
export function AppraisalPage({
  ...
  const landValue   = Math.round(grossValue * 0.60);
  const buildValue  = Math.round(grossValue * 0.40);
  ...
  const placeText = [deed?.amphurName, deed?.provinceName].filter(Boolean).join(' ') || '-';
  const notes = [
  ...
  return (
    ...
    >
      ...
      <PdfView style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
        <PdfView>
          <PdfText style={{ fontWeight: 700, fontSize: 9 }}>หลักทรัพย์ : {loan.loanNumber || '-'}</PdfText>
          <PdfText style={{ marginTop: 2, fontSize: 8.5, color: T.muted }}>คำรับ : วางหลักทรัพย์จำนอง</PdfText>
        </PdfView>
      ...
      <PdfView style={{ flexDirection: 'row', gap: 12 }}>
        ...
        <PdfView style={{ width: '57%' }}>
          ...
          <FieldsetBox label="ผลการประเมินมูลค่า" style={{ marginBottom: 8 }}>
            ...
            <PdfView style={{ flexDirection: 'row', paddingVertical: 4, borderBottomWidth: 1, borderBottomColor: T.borderLight, borderBottomStyle: 'solid' }}>
              <PdfText style={{ width: '42%', fontSize: 8.5, color: T.label, fontWeight: 700 }}>รวมมูลค่าทรัพย์สิน</PdfText>
              <PdfText style={{ flex: 1, fontSize: 8.5, color: T.text, fontWeight: 700 }}>฿{formatCurrency(totalGross)}</PdfText>
            </PdfView>
            <PdfView style={{ flexDirection: 'row', paddingVertical: 4, borderBottomWidth: 1, borderBottomColor: T.borderLight, borderBottomStyle: 'solid' }}>
              <PdfText style={{ width: '42%', fontSize: 8.5, color: T.label }}>หัก ค่าเสื่อมราคา ({(depRate * 100).toFixed(0)}%)</PdfText>
              <PdfText style={{ flex: 1, fontSize: 8.5, color: '#dc2626' }}>-{formatCurrency(depAmount)}</PdfText>
            </PdfView>
            <PdfView style={{ backgroundColor: T.highlight, borderWidth: 1, borderColor: '#bfdbfe', borderStyle: 'solid', borderRadius: 3, padding: 7, marginTop: 5 }}>
              <PdfView style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                <PdfText style={{ fontSize: 10, fontWeight: 700, color: T.text }}>มูลค่าประเมินสุทธิ</PdfText>
                <PdfText style={{ fontSize: 17, fontWeight: 700, color: T.blue }}>฿{formatCurrency(netValue)}</PdfText>
              </PdfView>
              <PdfText style={{ fontSize: 7.5, color: T.label, marginTop: 2, textAlign: 'right' }}>
          ...
          <FieldsetBox label="สรุปการเปรียบเทียบตลาด">
            <PdfView style={{ flexDirection: 'row', backgroundColor: T.bgHeader, paddingVertical: 4, paddingHorizontal: 4, borderBottomWidth: 1, borderBottomColor: T.border, borderBottomStyle: 'solid' }}>
            ...
            {compareRows.map(([no, name, area, price], idx) => (
              <PdfView key={no} style={{ flexDirection: 'row', paddingVertical: 4, paddingHorizontal: 4, backgroundColor: idx % 2 === 0 ? T.bg : T.bgGray, borderBottomWidth: 1, borderBottomColor: T.borderLight, borderBottomStyle: 'solid' }}>
                ...
                <PdfText style={{ flex: 1, fontSize: 8, textAlign: 'right', color: T.text }}>{formatCurrency(price)}</PdfText>
              </PdfView>
            ))}
            <PdfView style={{ flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 4, paddingTop: 4 }}>
              <PdfText style={{ fontSize: 8.5, fontWeight: 700 }}>ค่ากลางตลาด (Median)</PdfText>
              <PdfText style={{ fontSize: 8.5, fontWeight: 700, color: T.blue }}>฿{formatCurrency(medianValue)}</PdfText>
            </PdfView>
          </FieldsetBox>
        </PdfView>
        ...
        <PdfView style={{ width: '41%' }}>
          <FieldsetBox label="แผนผังที่ดิน" style={{ marginBottom: 8 }}>
            <PhotoBox src={deedImageUrl} label="แผนผังที่ดิน" style={{ height: 110 }} />
          </FieldsetBox>

          <FieldsetBox label="รายละเอียดการประเมิน" style={{ marginBottom: 8 }}>
            <PdfView style={{ flexDirection: 'row', backgroundColor: T.bgHeader, paddingVertical: 4, paddingHorizontal: 3, borderBottomWidth: 1, borderBottomColor: T.border, borderBottomStyle: 'solid' }}>
            ...
            <PdfView style={{ flexDirection: 'row', paddingVertical: 4, paddingHorizontal: 3, borderBottomWidth: 1, borderBottomColor: T.borderLight, borderBottomStyle: 'solid' }}>
              <PdfText style={{ width: '32%', fontSize: 7.5 }}>ราคาที่ดิน</PdfText>
              <PdfText style={{ width: '22%', fontSize: 7.5, textAlign: 'right', color: T.muted }}>{sqWah ? `${sqWah} ตร.ว.` : areaText}</PdfText>
              <PdfText style={{ width: '22%', fontSize: 7.5, textAlign: 'right', color: T.muted }}>{pricePerSqWah ? formatCurrency(pricePerSqWah) : '-'}</PdfText>
              <PdfText style={{ flex: 1, fontSize: 7.5, textAlign: 'right' }}>{formatCurrency(landValue)}</PdfText>
            </PdfView>
            <PdfView style={{ flexDirection: 'row', paddingVertical: 4, paddingHorizontal: 3, borderBottomWidth: 1, borderBottomColor: T.borderLight, borderBottomStyle: 'solid' }}>
              ...
              <PdfText style={{ width: '22%', fontSize: 7.5, textAlign: 'right', color: T.muted }}>-</PdfText>
              <PdfText style={{ flex: 1, fontSize: 7.5, textAlign: 'right' }}>{formatCurrency(buildValue)}</PdfText>
            </PdfView>
            <PdfView style={{ flexDirection: 'row', paddingVertical: 4, paddingHorizontal: 3, borderBottomWidth: 1, borderBottomColor: T.borderLight, borderBottomStyle: 'solid' }}>
              ...
              <PdfText style={{ flex: 1, fontSize: 7.5, textAlign: 'right', color: '#dc2626' }}>-{formatCurrency(depAmount)}</PdfText>
            </PdfView>
            <PdfView style={{ flexDirection: 'row', paddingVertical: 4, paddingHorizontal: 3 }}>
              <PdfText style={{ width: '32%', fontSize: 7.5, fontWeight: 700 }}>รวม</PdfText>
              <PdfText style={{ width: '22%', fontSize: 7.5, textAlign: 'right' }} />
              <PdfText style={{ width: '22%', fontSize: 7.5, textAlign: 'right' }} />
              <PdfText style={{ flex: 1, fontSize: 7.5, fontWeight: 700, textAlign: 'right', color: T.blue }}>{formatCurrency(netValue)}</PdfText>
            </PdfView>
          </FieldsetBox>

          {/* รูปถ่าย: 1 ใหญ่ + 2 เล็ก */}
          <FieldsetBox label="รูปถ่ายทรัพย์สิน" style={{ position: 'relative', marginBottom: 8 }}>
          ...
          <FieldsetBox label="หมายเหตุ">
            {notes.map((note, i) => (
              <PdfView key={i} style={{ flexDirection: 'row', marginTop: i === 0 ? 2 : 3 }}>
                <PdfText style={{ fontSize: 7.5, color: T.muted, marginRight: 4 }}>•</PdfText>
                <PdfText style={{ flex: 1, fontSize: 7.5, color: T.muted, lineHeight: 1.35 }}>{note}</PdfText>
              </PdfView>
            ))}
          </FieldsetBox>
        </PdfView>
      ...
      {/* ── SIGNATURE ── */}
      <PdfView style={{ flexDirection: 'row', justifyContent: 'flex-end', marginTop: 10 }}>
        <PdfView style={{ width: '52%', flexDirection: 'row', gap: 20 }}>
          <PdfView style={{ flex: 1, alignItems: 'center' }}>
            <PdfText style={{ fontSize: 8, color: T.muted, marginBottom: 16 }}>ลงชื่อ</PdfText>
            ...
            <PdfView style={{ width: '100%', borderBottomWidth: 1, borderBottomColor: T.borderLight, borderBottomStyle: 'dashed', marginTop: 7, marginBottom: 3 }} />
            <PdfText style={{ fontSize: 7.5, color: T.muted }}>วันที่ ......../........../..........พ.ศ.</PdfText>
          </PdfView>
          <PdfView style={{ flex: 1, alignItems: 'center' }}>
            <PdfText style={{ fontSize: 8, color: T.muted, marginBottom: 16 }}>อนุมัติโดย</PdfText>
            <PdfView style={{ width: '100%', borderBottomWidth: 1, borderBottomColor: T.text, borderBottomStyle: 'solid', marginBottom: 4 }} />
            <PdfText style={{ fontSize: 8, color: T.muted }}>(...................................)</PdfText>
